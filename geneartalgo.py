...
def optimize_sequence_sliding_window(amino_acid_sequence, codon_df,target_gc_content):
    ...
    while i < total_steps:
        ...
        possible_codons = [
            codon_df[codon_df['Amino'] == aa]['Codon'].tolist()
            for aa in window
        ]
        ...
        # Select the best scoring combination
        best_combination, best_score = max(scored_combinations, key=lambda x: x[1])

        # Fix the first codon of the best combination
        optimized_sequence += best_combination[0]
...
while algorithm is None:
    ...
    print("7 - GENEOptimizerApproach (CPU intensive)")
    try:
        algorithm = int(input("Please choose an optimization algorithm:"))
    except ValueError:
        print("please provide a valid option")
        continue
    if algorithm != 1 and algorithm != 2 and algorithm != 3 and algorithm != 4 and algorithm != 5 and algorithm != 6 and algorithm !=7:
        print("please provide a valid option")
        algorithm = None
    else:
...
while further_optimization:
    ...
    if algorithm >3 and algorithm <7: 
        print("10 - use deoptimized codon usage for the above tools")
    ...
    if further_optimization_choice == 0:
        ...
        gc_content = calculate_gc_content(optimized_dna_sequence)
        print(f"GC Content of optimized DNA sequence: {gc_content:.2f}%")
    elif further_optimization_choice == 10 and algorithm >3 and algorithm <7:
        use_inverted = input("Do you want to invert the codon usage table ? y/n").strip().upper()
        if use_inverted == "Y":
            codon_df['Frequency'] = codon_df['Inverted_Frequency']
            print("Codon usage inversion applied")
